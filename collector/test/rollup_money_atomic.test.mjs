...
/* This is the guarantee that sharing a transaction most easily destroys: in
   Postgres the first error aborts the whole transaction and every later
   statement fails with "current transaction is aborted", so five INSERTs in
   one transaction without savepoints turn one missing table into an empty
   provenance record. Asserted by genuinely breaking one source — the park
   ledger — rather than by simulating the error, because the thing being
   tested is the database's behaviour and not the code's bookkeeping. */
...
check('the broken one is absent rather than fabricated', !srcs.includes('yango_park_ledger'),
  JSON.stringify(srcs));
check('the transaction survived the error and committed',
...
check('and the source comes back the moment its table does',
  (await q(`SELECT count(*)::int n FROM money_event WHERE source = 'yango_park_ledger'`))[0].n === 1);
