...
| P23 | **A screen that could not load, drawn as critical (component).** `failed()` builds an untitled `.m-card.m-err`. The rule that gives an untitled card a hairline, `.m-card:not(:has(>h2))` (specificity 0,4,1 with the scope), outranked the `.m-err` rule's 3px negative rule (0,4,0). So every failed read under the skin drew a 1px grey line where ruling 1 asks for CRITICAL. The solid dot before the words was right; the rule over them was not. Measured on Safety's failed summary: 1px, grey-2. The same rule draws every `failed()` block on every screen. Restated in m/arkiv-m.css §18 at a weight that wins | **written** | `phone_arkiv` §4.7 (new check: 3px, the negative colour, and a solid dot). Revert (the rule removed) → 1 (`1px rgb(214, 214, 217)`). File restored, md5 checked |
| P24 | **Driver.** 00 At a glance heads the statement: the face as a square plate, the name, the channels and days. Contact follows as part of the statement, unnumbered and on a hairline, and still dials (`tel:`) and mails (`mailto:`). Reaching the driver stays the first thing under their name. The tiles follow, with Bookings as the hero. 01 onwards are the sections that were there, in their order: Bookings a day, Against the other N, Harsh driving, More. **Reasons in the value slot:** the money tiles come from ui.js helpers that return '—' with the true reason as their sub-line, and under the skin that sub-line takes the value slot. **Found and fixed under the skin:** (a) Distance printed "— km" when `km` was NULL. That is a dash with a unit glued on, which the absent test cannot see, so it read as a figure. It is now absent with avgKmSub()'s reason. (b) Completed with no outcome recorded printed a dash with a WARNING tone, because `n(null) >= 80` is false. It now says "no booking in this window records whether it was completed" (the SQL's `nullif(count(*) FILTER (WHERE outcome IS NOT NULL), 0)`) and carries no dot. (c) An unmeasured harsh-driving rate says the server's reason, not "not measured". (d) A FAILED read: `/api/driver/kpis`, `/standing` and `/quality` each `.catch(() => null)`. A failed kpis read then printed every money tile with "no platform statement and no priced booking covers this window", which is a reason but not the true one. The head read "— bookings in this window". A failed standing or quality read removed its section without a word. Under the skin, each says it could not be fetched, in its own place, and the head says so too. **The old phone keeps (a), (b) and (d) until the flip, and a test pins that it does. Named for the operator** | **written** | `phone_arkiv` §4.18 (16). The checks cover the recording, a copy with the figures NULL and the rate unmeasured, and a copy with the three reads refused; the money tiles are compared with the desktop helpers' output for the same answer. Reverts: no 00 → 1 · Contact numbered → 2 · no reasons from the helpers → 1 · "— km" → 2 · a warning on absent Completed → 1 · "not measured" → 1 · a failed kpis read silent → 2 · the sections vanishing → 1 · the dash subtitle → 1 · no hero → 2. (The first no-hero mutation hit Today's identical line and proved Today's hero instead; re-run against the driver's own line.) Files restored, md5 checked. phone_classic 34, phone 144, driver_money_tiles 82, driver_standing 37, driver_photo 39 |
| P25 | **Vehicle.** The driver screen's form, for a car. 00 At a glance heads the statement, whose claim is the plate, set in mono because it is a code read character by character. The tiles follow, with Bookings as the hero, then the sections that were there, numbered: Bookings a day, Harsh-driving events, Who drove it, More. **Found and fixed under the skin:** (a) Fares' "AED 36.40 a booking" sat beside a Bookings tile of every booking, but `avg_fare` is `avg(price) FILTER (WHERE has_fare)` (api/vehicle_routes.js): the average over the bookings that carry a fare. It is the same denominator defect avgKmSub() exists for, and it is now said the same way: "… a booking over the N that report a fare", where N is not every booking. (b) With no fare, Fares is absent with the desktop vehicle page's own reason, "no booking on this vehicle carries a fare". Before, it was a bare dash. (c) Distance printed "— km"; it is now absent with avgKmSub()'s reason. (d) Main Power Lost, the tracker reporting its own power loss, was drawn as one more harsh-driving bar. It is now marked "(tracker fault)" in the bar's label, with the desktop's own caption and counts. The desktop's closing clause about a rate is left out, because this screen shows no rate. (e) Failed kpis, safety and drivers reads: a failed kpis read printed "—" and "— km", and a failed safety or drivers read removed its section. Each now says it could not be fetched, under its own head. **The old phone keeps (a)–(e) until the flip. Named for the operator.** Not added: the desktop vehicle page's Money in, Utilisation and Idle days tiles. The phone never had them, and the design note keeps the phone's scope | **written** | `phone_arkiv` §4.19 (12). The checks cover the recording, a copy with no fare and no distance, and a copy with the three reads refused. Reverts: no mono plate → 1 · the fare denominator unnamed → 1 · the tracker fault unnamed → 1 · a bare dash for Fares → 1 · "— km" → 1 · a failed kpis read silent → 2 · safety vanishing → 1 · drivers vanishing → 1 · no hero → 2 · no 00 → 1. Mutations targeted the vehicle's own lines, because the driver's are textually identical. Files restored, md5 checked. phone_classic 34, phone 144, tokens 149 |
...
### P69 · #safety (people, vehicles, events)

| page | what changed under the skin | state | proof |
|---|---|---|---|
| #safety, #safety/vehicles, #safety/events | An `ak` flag in V.safety. **One 00 band shared by the three tabs:** the verdict is the statement ("552 per tracked vehicle" stays its figure). NEW *Worst rate on the road* is the hero: the top driver rated over 200 km, with a worded gap against the fleet's /api/kpis rate where up is worse. NEW *Fleet events per 100 km* has the median rated driver in its sub-line. Driving events, Device faults, Vehicles involved and Drivers named follow, untoned, with both cohort links kept. "Events nobody held the car for" moves to the band's note with its figure. **People:** "Who drives hardest" is in the FMS/InfoTrack identity (every event is from that alert feed), the fleet rate is named in its caption, and the 200 km floor, the bars, the km and the click are unchanged. NEW "The shape of the rated drivers": up to six equal bins up to a round ceiling over the worst rate. The driver table and its default sort are unchanged. **Vehicles:** the worst-vehicles bars are in the identity. NEW "Above the fleet rate, car by car": each car's events per 100 km the feed covered (/api/vehicles/directory, a second fetch that holds nothing up) over 200 km, highest first, with how many are above the fleet rate. The vehicle table is unchanged. **Events:** the two donuts become one ranked list, driving kinds first in the identity and the tracker's faults after in ink ("fault in the tracker box"), each share in the caption. **†** on every tab: what counts as harsh (no threshold held), events nobody could place, drivers who cannot be rated (counted), and what actually happened (no outcome feed) | **written** | `test/arkiv_fleet.test.mjs` #safety 20/20, with a synthetic reversed directory. Revert-proofs (md5-restored): the unattributed tile back fails two checks; `invert` off makes "up is worse" fail; the people bars back on `--s8` fail; the per-car sort off fails "highest first"; one colour for the event kinds fails "in ink"; the first draft's fixed bins fail "the last reaching the worst rate". `arkiv_classic_frozen` ONLY=safety 7/7. `arkiv_fils` gains the three routes; the live scan is clean. Pinned: `safety_cap` 9, `device_fault` 36, `kpi_one_tile` 20, `cohorts` 74, `chart_marks` 151, `page_contract` 95 |

Caught on production and fixed before the commit: the draft's histogram bins were fixed at 0–5 … 80 or more. The production median rated driver runs at 94 per 100 km, so all but nine people fell in the last bin, a histogram with one bar. The bins now come from the data (0–50 … 150–200 on production).

Deviations:
* The fleet rate is named in the "Who drives hardest" caption, not drawn as a reference rule; hbars draws none.
...
### P72 · #map and #map/replay (one view, two modes, one commit)

| page | what changed under the skin | state | proof |
|---|---|---|---|
| #map, #map/replay | **Live (restyle only).** The toggle, the map, marker-click replay, the four tiles, the no-lock note and the permalink are all kept. A marker is its FEED's colour, and what the car is doing is a step of that feed's own ramp (map.js `rampOf`: `--c-<feed>-engaged` for a passenger aboard, `-available` for moving, `-idle` for stopped). A moving car with no seat reading keeps its feed's moving step with a dashed ink ring (the absence of a reading, never drawn as "empty"). A stale fix is a hollow ring in the absence outline, never red. A feed this product cannot name is grey. The legend has one row per feed on the map with its three steps, then the no-seat ring and the stale outline. **Replay.** The four tiles are untoned in a hero-less glance row. *Driver* is ABSENT when no custody record names anybody. *With passenger* "not measured" is an absence with its reason. The journey's lines are in the feed's ramp (occupied the identity, empty the moving step dashed, no reading grey dotted), and the last-fix ring is ink. **NEW, replay only and BELOW the map** so the controls, tiles and map keep their place: how long between one fix and the next (the ≥20-minute gaps that break the line in grey), what each fix said it was doing, how fast it was going (moving fixes only, the fixes with no speed counted in words), and this car's days (/api/map/days?plate=, since the fleet list is capped at 400: fixes a day and top speed a day, days with no trail as outlines, today unfinished, a day opens its replay). Back in live mode the marks go | **written** | `test/arkiv_fleet.test.mjs` #map 15/15, including a synthetic two-feed day with no custody. Revert-proofs (md5-restored): the old marker colours back make "every filled mark is a step of a feed's own ramp" fail; the Distance qualifier off fails; the replay marks off fail five checks; not clearing them in live mode fails "the replay marks go"; the unfiltered days list back fails "asked for by plate". `arkiv_classic_frozen` ONLY=map 3/3. **The golden caught one regression:** the marks' host had been appended in both skins, and it is now contract-only. Pinned: `routes` 65, `assets` 40, `nav_sections` 17, `window` 36, `timezone` 19, `page_contract` 95. Two pinned checks encoded the previous Arkiv pin and were updated to the plan's rule in this commit. `chart_marks` "a stale pin is a hollow ring": the stroke is now the absence outline, and the moving no-seat pin is the feed's moving step with a dashed ink ring instead of `--b300` (151/151). `tokens` "map and driver pins ring in --pin-ring": the regex now admits the no-seat ink ring (149/149) |

**Flagged, not ruled (plan: owner ruling needed): the replay's Distance.** On production one car-day read 5,361.7 km. The straight-line sum crosses between interleaved device streams. This is not fixed here. Under the contract, when the day's fixes came from more than one feed, the tile's sub-line now says so: "between fixes from 2 feeds (…), interleaved — a straight line drawn from one device's fix to the other's can overstate it; which figure is right awaits an owner's ruling". A one-feed day is unqualified.

Deviations:
* #map and #map/replay are one view in two modes, committed together.
* The replay's §02 reporting strip is carried by the gap histogram and the per-day fixes.
* The §06 driver's bookings are a caption sum, not a chart.

### P73 · #sources

| page | what changed under the skin | state | proof |
|---|---|---|---|
| #sources | An `ak` flag in V.sources. **00:** the verdict is the statement. Its "need attention" figure is not a tile (ruling 7). The tiles are untoned: *Failed on the last run* (the hero, with the partial and quiet-success counts in its sub-line), NEW *Days still owed* (collectionDebt's distinct days, from N refused windows, with how many are past retention; ABSENT when nothing was refused), NEW *Rows on record* (every dataset summed), and NEW *Stalest scheduled feed* (with the freshest named). **NEW 01** why the windows were lost: every refused window by what the provider said, classed from its own sentence (past retention, credential refused, host never answered, rate-limited, fewer rows, refused with no reason given, another reason), and by provider in channel colour. **NEW 02** when each provider lost a window: refusals by starting month, one plot per provider on one ruler. The collector-health table and all its columns are kept. A run's status is a word: an error is the red "!" pill, a partial an ink outline chip, ok plain grey, and "healthy" plain words with no green dot. The data coverage table is kept. **NEW** when each feed last wrote (hours since the newest run, a source nothing schedules in grey) and rows on record by source (channel colour). The windows that did not land are kept, with "outstanding" in the negative colour as text and "gone for good" an outline chip, then the pre-built summaries. The coordinates table is kept, and its caption names each source once. The census gains a *Stored as* column (the payload's `mapped_to`, fetched and never shown). **†:** what a provider holds and never sent, days no provider will serve again (counted), and why a provider refused (its own words) | **written** | `test/arkiv_fleet.test.mjs` #sources 17/17, with a synthetic census carrying `mapped_to`. Revert-proofs (md5-restored): the old status tag back fails "status words"; the non-deduplicated caption fails "names each source once"; the summaries left before the windows fail "order"; the Stored-as column off fails; the old standing tags fail "windows"; the green healthy dot back fails "plain words". `arkiv_classic_frozen` ONLY=sources 2/2 (the old skin keeps its caption, defect and all). The live scan is clean. Pinned: `source_value` 10, `slow_skeleton` 9, `tracker_speed` 28, `collection_debt` 34, `fms_split` 14, `uber_profile` 41, `chart_marks` 151, `page_contract` 95, `kpi_one_tile` 20 |

Caught on production and fixed before the commit: the first reason classes left 83 of 236 refusals as "another reason". FMS's own sentence is "refused: <endpoint> failed", 81 of them, a refusal that names no reason, and it is now said as exactly that. One Uber rate limit and one Bolt "collected 1196 of 1197 declared" now have their own classes.

Deviations:
* **"Windows that did not land" is NOT folded.** The plan marks the fold OPTIONAL and the operator's call; it is left for the operator.
* The plan's §02 small multiples share one ruler through barChart's `max`.
* The old skin still prints "uber and uber send addresses as text". The golden holds it; only the new skin's caption is fixed.

### P74 · #coverage

| page | what changed under the skin | state | proof |
|---|---|---|---|
| #coverage | **The plan's "BUG FIRST"** (an unguarded await of /api/coverage) was already fixed by STEP 3 (fff8be1): coverage.js checks `alive(gen)` after that await. Confirmed here and left as it is. An `ak` flag in renderCoverage. **00:** the verdict over the whole record is the statement. Its figure, the sources with a hole, folds into it by name (ruling 7). The tiles are untoned: Missing days over all history (the hero), Sources with data, Missing days in the window (ABSENT "the windowed calendar could not be read" when it could not be), and Rows on record. **NEW 01** the window's rows day by day, every source summed: a day nothing was collected on is an outline, today unfinished, a day opening #day. **02** the Uber check: its four tiles untoned in a hero-less row, with Agreement ABSENT if no month was checked. NEW scatter of Uber says against we hold, one dot per window, with the line where they agree. The table is kept, its week tag an outline chip. **NEW 03** rows we hold that Uber's report does not list, per window (`ours_only`). Trips with no earnings are kept. **NEW 04** days with a reading, per dataset. The shared-silence table is kept. **Every per-source calendar and gap table is kept.** Each "was it asked for?" answer, "a request inside it failed" included, is an ink outline chip (absence is never a semantic hue, L5.9). Today's calendar cell is hatched (arkiv.css `.cal .c.today`, additive; the gap outline was already STEP 2's). **†:** bookings with no money (counted from `earnings_gaps`), ratings before we first asked (the rating horizon's date), and a second count for any channel but Uber (only Uber serves one) | **written** | `test/arkiv_fleet.test.mjs` #coverage 14/14, including a synthetic unreadable window. Revert-proofs (md5-restored): the fold off makes ruling 7 fail; a hero in the Uber row makes "one hero on the page" fail; the toned tags back make "an ink outline chip" fail; the scatter off fails; the window chart off makes two checks fail. `arkiv_classic_frozen` ONLY=coverage 2/2. The live scan is clean. Pinned: `window_honesty` 14, `calendar_window` 82, `fms_split` 14, `nav_sections` 17, `chart_marks` 151, `page_contract` 95, `kpi_one_tile` 20, `arkiv_skin` 111 |

Deviations:
* The "days the ledger missed" absence cell the plan suggests is not built: the ledger is event-driven, and a day with no import is not a miss (the page's own "not a daily feed").
* The calendar's collected cells keep their ink-by-opacity intensity, which is already achromatic and sequential.

### P75 · #providers and one field's page

| page | what changed under the skin | state | proof |
|---|---|---|---|
| #providers, #providers/<provider>/<surface>/<key> | An `ak` flag in renderProviders. **00:** the verdict is the statement. Its figure (the refused surfaces, or the fields dropped when nothing refused) folds out of the tiles by name (ruling 7). "Providers not configured" moves to the band's note. The rest are untoned: Surfaces probed, Answering, Fields we are not keeping (the hero while anything refused), and Last probe (ABSENT "the probe has never run" when it has not). **NEW** fields not kept, by provider: ranked, in each provider's colour, a bar scrolling to its panels. The jump list, the unkept-only toggle, the refusal table (its status an outline chip) and every surface panel are kept. Each heading carries the provider's swatch (arkiv.css spacing, additive). The collector status is a chip (error the red "!" pill). The not-kept chips are plain, KEPT/NO are outline chips, and the filled share is a small bar beside its figure (arkiv.css `.pv-fill`, additive). **†**, redrawn once with the panels on every toggle: what a refused surface would send (counted), fields past the probe's sample, and providers we did not ask. **One field's page:** restyle only | **written** | `test/arkiv_fleet.test.mjs` #providers 12/12. Revert-proofs (md5-restored): the fold off fails ruling 7; the heading swatch off fails; the toned KEPT/NO pills fail; the by-provider chart off fails; the † band not marked for redraw fails "the † band once" (it doubled on the toggle). `arkiv_classic_frozen` ONLY=providers 3/3. The live scan is clean. Pinned: `server_redaction` PASS, `signed` 14, `nav_sections` 17, `chart_marks` 151, `page_contract` 95, `kpi_one_tile` 20, `arkiv_skin` 111, `tokens` 149 |

**Found, not fixed (it would change the old skin too; for the operator):** `answered()` treats a 200 whose body is only error-ish keys as a refusal, but its key list (`error|message|fault|status|code`) misses Bolt's `error_hint`. On production, "Bolt · ecosine:getDrivers" answers HTTP 200 with 0 records and the fields `code`, `error_hint` and `message`, and is counted in "Answering 20 of 28". It is a refusal wearing a success code, the case the function's own comment describes. The fix is one word in a function both skins share, so the old skin's golden would move; it is left for a decision.

### P76 · #settings

| page | what changed under the skin | state | proof |
|---|---|---|---|
| #settings | An `ak` flag in V.settings. **00 above the paste box** (plan §4: one screen at most): Credentials expired (the hero; its sub names up to three and says the collector is being refused), Keys configured (held here or by the collector that uses them, of all defined), Next to expire (days, or hours under one day, with the credential and its key; ABSENT "no credential still in force carries an expiry this page can read" when none does), Keys not set, Held in the environment (changing one needs a redeploy). The form's own four-tile row is the band now, so it is not printed twice. **01 NEW** days left on each credential that carries an expiry: expired first, then soonest; an expired key draws no bar and its value slot says how long ago it died ("expired 28 d ago"), not the "0" hbars prints for a zero. Below, everything keeps its order and structure: the paste box, the drop zone, the admin token, every credential row, all five run buttons and the runs table. Restyle only there: each source and "unset" an outline chip, an expired credential the red "!" pill, days left in ink (bold within a week, grey after), the runs' State untoned except failed (the red "!"), Asked-by an outline chip, and a wait over an hour in bold rather than amber. **†:** whether a set key is accepted (only the collector's next run proves it), and when a key with no token date expires (a cookie or API key carries none) | **written** | `test/arkiv_fleet.test.mjs` #settings 10/10 (synthetic expiries through a fixture: one expired three days ago, one with five days left). Revert-proofs (md5-restored): the form's tile row printed again fails "the form kept whole … folded into the band"; the source tag toned again fails "every source an outline chip"; the how-long-ago words removed fails the days-left check; the days-left panel after the paste box fails the order check; the value-slot rewrite off fails "prints no "0"". `arkiv_classic_frozen` ONLY=settings 2/2. Pinned: `credential_visibility` 19, `uber_oauth_paste` 36, `credential_save_check` 108. The live scan is clean |

The literal line `return '<span class="tag warn">unset</span>';` in the source tag is kept verbatim, because `credential_visibility` reads the source for it. The contract branch is the line before it.

### P77 · #notfound

| page | what changed under the skin | state | proof |
|---|---|---|---|
| #notfound (any address that names no page) | Restyle only (plan §4 #notfound). Under the contract the notice is a plain `.note`, ink with no dot, instead of `.note.err`. A missing page is an absence, not a worse number, so it wears no red. The sentence, the closest destinations (matched on id and label) and the fallback link to Money per car and driver are the same in both skins. The coverage.js late-panel leak the plan names was already guarded by `alive(gen)`, and the test checks that nothing else draws under the notice | **written** | `test/arkiv_fleet.test.mjs` #notfound 7/7 (the old skin keeps `.note.err`; the Arkiv notice has no error class and no `::before` dot; the sentence and fallback link are whole; one panel only; #hotels still names Corporate & hotels; 390 does not scroll). Revert-proof (md5-restored): `note err` in both skins fails the ink-notice check. **`arkiv_classic_frozen` gains `zzz-not-a-page` in EXTRA**, recorded from the base tree `abb79ad` (`RECORD=1 PUBLIC_DIR=… ONLY=zzz-not-a-page`), 2/2. Its revert (`note` in both skins) fails the golden. Pinned: `nav_sections` 17 |

### Fleet and Sources, the section's full suite

The suite gave `328 files, 11873 assertions, 1 file(s) failing` on a private mock (`MOCK … GONE` printed), at 12:51 UTC, outside the evening window. The one failure was a test whose premise P76 changed, not a defect in a page. It is fixed below. Nothing else failed.

| fix | what | state | proof |
|---|---|---|---|
| P76a · page_contract footer-clearing check | "…and the next page does not wear the last one's basis or colophon" navigated from #overview to **#settings** and required an empty colophon there. P76 gave #settings a colophon of its own ("credentials and on-demand runs · 5 keys" on the mock), which is right for that page, so the check failed on a page that was doing its job. The check proves that the foot is CLEARED between pages, so the next page must be one that writes no foot. It now goes to an address that names no page (#notfound writes no basis and no colophon in either skin), and a comment says why | **written** | `page_contract` 95/95 alone against a private mock. Revert-proof, run on the page and not on the test: with `clearPageFoot()` emptied in ui.js, the check fails (the not-a-page view wears #overview's "Built from Uber …" basis), and the md5 is restored |

### The page phase's final full suite (2026-09-24)

`328 files, 11872 assertions, 1 file(s) failing` on a private mock (`MOCK … GONE` printed), finishing 13:10 UTC. That is outside the evening window: `payout_scope` 42/42 and `phone_today_only` 19/19 passed. The one failure was `money_contradictions` (2 checks). It is a defect in a shared function that predates the phase, not in a converted page. It is fixed below, and the full suite was NOT re-run after the fix (one run at the end).
...
| # | what | state | proof |
|---|---|---|---|
| B1 | The name everywhere the product names itself: the tab title, the iOS home-screen title, the manifest (name and short_name), the old skin's rail, the Arkiv masthead, the phone's More masthead, and every phone tab title ("Today · FleetMirror"). "Fleet" as a section and a column name is not the brand and stays | **proven** — `e79bf05`, deployment `f81be41` (ACTIVE 02:32:14Z, 2026-09-25) | `brand` §1 (10 checks) |
| B2 | The mark. `brand/` holds the operator's three originals as given, plus `mark.png` (the transparent file cropped to its own box) and `mark-dark.png` (the same shapes with the black half in the artwork's white; the artwork has three exact colours, so this is a swap, not a redrawing). The five app icons are the on-dark file resized, by bin/make-icons.mjs (rewritten: it used to DRAW the old icon), under NEW names (`fleetmirror-*.png`), because /icons is served immutable for a year and a picture replaced under its old name stays old in every browser that fetched it. The mark sits inside the maskable safe circle (farthest corner 395px of 409.6). The manifest's colours are the artwork's own `#151513` | **proven** — `e79bf05`, deployment `f81be41` (ACTIVE 02:32:14Z, 2026-09-25) | `brand` §3: sizes, and colours read pixel by pixel; identical alpha for the two marks; every icon address new and the old names gone. Pointing the favicon back at `icon-192.png` fails it |
| B3 | The theme picks the mark: `--logo` in the old light `:root` and in both old dark blocks, so the four theme states and both skins follow it. The service worker precaches both marks | **proven** — `e79bf05`, deployment `f81be41` (ACTIVE 02:32:14Z, 2026-09-25) | `brand` §4: four theme states × two skins in a browser. Reverts fail: `--logo` dropped from the dark blocks fails 5; the old rail restored fails 6 |
| B4 | The phone's header shows the mark alone, with the name for a screen reader (`.sr`) and a 44px target. With the whole word, "Cash handed in" and "Unauthorized" were cut at 360px. The truncation measured at 320/360/375/390 is now LESS than before the rename (57/6/0/0px against 97/26/11/0), because the mark is narrower than "Fleet" was | **proven** — `e79bf05`, deployment `f81be41` (ACTIVE 02:32:14Z, 2026-09-25) | `brand` (phone at 360); `phone_arkiv` 255 (the tap-target sweep) |
...
**On production, 2026-09-25.**
- **Served.** `<title>FleetMirror</title>`, the iOS title FleetMirror, and the
  rail's `<b>FleetMirror</b>`. The manifest names FleetMirror with `#151513`
  and points only at `fleetmirror-*.png`.
- **Caching.** The five icons answer 200 with `max-age=31536000, immutable`;
  `brand/mark.png` and `mark-dark.png` answer 200 with the five-minute
  revalidating rule. The old `/icons/icon-192.png` no longer answers with an
  image (it falls through to the page).
- **Screens.** Through bin/prod-mirror.mjs (assets byte-identical): the old
  rail, the Arkiv masthead, and the phone at 390 and 360. In light and dark,
  each shows the right mark beside, or for a screen reader behind,
  "FleetMirror", with no sideways overflow.
- **Suite before the deploy:** 332 files, 12,214 assertions, 0 failing.
- **A lost first suite run.** It was started alongside a command that changed
  the shared shell's directory, so its test server launched from the repo root
  and died ("Cannot find module …/mockapi.mjs"). Its wait loop had no time
