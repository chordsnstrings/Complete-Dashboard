...
| B2 | The mark. `brand/` holds the operator's three originals as given, plus `mark.png` (the transparent file cropped to its own box) and `mark-dark.png` (the same shapes with the black half in the artwork's white; the artwork has three exact colours, so this is a swap, not a redrawing). The five app icons are the on-dark file resized, by bin/make-icons.mjs (rewritten: it used to DRAW the old icon), under NEW names (`fleetmirror-*.png`), because /icons is served immutable for a year and a picture replaced under its old name stays old in every browser that fetched it. The mark sits inside the maskable safe circle (farthest corner 395px of 409.6). The manifest's colours are the artwork's own `#151513` | **written** | `brand` §3: sizes, and colours read pixel by pixel; identical alpha for the two marks; every icon address new and the old names gone. Pointing the favicon back at `icon-192.png` fails it |
