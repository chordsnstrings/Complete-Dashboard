...
- STEP 2, M. Charts: colour by name, the mark specs, the outline/hatch swap together with its 9 caption rewrites, donut rendered as bars, graphite heatmap. This JS works under both skins through the token aliases. One commit per chart function, each shown with a screenshot, and each fix proven by reverting it and watching its test fail.
  - *As built, 2026-09-23 (branch reskin-foundation; details in FIX-STATUS.md "Arkiv reskin, STEP 2", rows CH1-CH10).* What later steps need to know. (1) **The chart FORM is a token.** Colour already followed the skin through `var()`; form could not (which element a mark is, its thickness, a hatch or an outline, the caption naming it). app.css's old `:root` declares today's forms as `--mk-*` (fit 0, max 96, end 3, base 3, gap 0, steps 7, absent hatch, unfinished hollow, projected solid, behind outline); the Arkiv block declares SPEC's from tokens.js `MARK`; `charts.js markForm()` reads whichever is in force — a token, never the skin attribute. A caption that names a treatment asks `drawnAs(kind)` / `drawnNoun(kind)`; a literal "hatched" is a bug (test/chart_marks.test.mjs scans for the old phrases). (2) **Colour by name.** `CAT` is eight slots, `--cat-1..8` (the old skin's `--s` colours; Arkiv's distinct graphite steps). `byName()` gives a datum whose label names a channel `--chan-<key>`, declared only in arkiv.css, with its slot as the `var()` fallback, so the old skin keeps its by-position colours until the flip. `sourceToken()` still returns null for an unknown key (returning `--grey` would repaint the old skin); the charts turn that null into `--chan-none`, grey under Arkiv. (3) **Donut.** The ring stays the default (ruling 6); `donut(…, { as: 'bars' | 'bar100' })` is ready for the page phase to ask for, page by page. (4) **Deviations from this plan, each for a stated reason:** the dashed line at charts.js:581 is areaChart's GAP connector, not a reference line, so under SPEC §5 it is hidden rather than made solid; gapBars' measure BEHIND a bar is a wash under Arkiv, not an outline (an outline means "not measured"); the rating spark is restyled from arkiv.css only; `spark()` stays in m/ui.js (the review's option, and STEP 3's to move); map pins are untouched (flagged for the map page); vehicle.js:215 is a comment, not a caption, on a file the concurrent unauthorized-trips branch edits. (5) **Both skins, on purpose:** five captions that stated a treatment the chart did not draw (#forecast "Hatched bars are forecast", #causes "Hatched columns are months we hold no data for", "Darker = busier" ×3 which is false in dark mode, #unit/assets "below that line" over a scatter with no line) and two heatmap callers that turned "not measured" into 0 in a tooltip. No mark moved in the old skin.
- STEP 3, M. The page-contract components in ui.js (glance, delta, spark, highlight, secHead, absenceBand, pageFoot), the shell footer with the principle line, and the four new render-audit checks. One pilot page (#overview) adopts them under the skin.
  - *As built, 2026-09-23 (branch reskin-foundation; details in FIX-STATUS.md "Arkiv reskin, STEP 3").* What the page phase needs to know. (1) **Which DOM a page builds is a token**, `--pg-contract` (app.css 0, arkiv.css 1), asked by `ui.js contract()` — the chart form's pattern, never the skin attribute. A page converts by branching on it, as #overview does (`overviewContract` / `overviewClassic`, sharing `overviewTiles`, `overviewVerdict` and `topDrivers`, so the two cannot disagree about a figure). (2) **The components**, all in ui.js: `glance(host, tiles)` builds its tiles with `kpiTile` (a `glance` flag adds `tile`/`t-*`, a channel swatch, `na` — the reason in the value slot —, `delta`, `spark`; a kpiRow tile is byte-identical to before); `delta(value, {invert, unit, of, kind, d, na})` (colour and word from semanticOf, arrow and sign from the arithmetic, so a fall in cancellations is a green ▼ −; `kind: 'level' | 'gap'` without `of` throws, ruling 4; `na` is printed verbatim, e.g. "not compared: …"); `highlight(node, token)` (wraps the figure in `span.hl`, refuses a 4th on a page, a 2nd in a band, svg, tbody and absent figures, returns `{applied, why}`); `secHead(idx, name, note)` (`idx` null takes the next number from the panel counter); `absenceBand(host, cells)`; `pageFoot({basis, colophon}, host)` and `clearPageFoot()`; `swatch(name)`; `tableFrom(…, {pairs: [[a, b], …]})`. `spark()` now lives in charts.js (m/ui.js re-exports it) and draws a null as a gap. gapBars takes `secondaryLine: {color, label}` (a step line with a direct label). (3) **The shell footer** (`#pageFoot`, after #view, inside #app) prints the principle line under the skin and is `display:none` in the old skin; under the contract `stampSource` puts `.srcline` in its basis. On the phone a host outside #view gets its own `.pf-inline` footer. (4) **The stale-render guard is central**: `render()` replaces #view with a fresh copy on every render; coverage.js:289 also checks `alive(gen)`. (5) **render-audit** gained `highlight-budget`, `grey2-text`, `off-token-colour`, `semantic-no-glyph`, the last three only where `--pg-contract` is 1. (6) **#overview deviations from its entry, each for a reason:** no §06 AED per km per tier (ruling 5); the verdict stays the 00 statement ABOVE the tiles (ruling 7); deltas on Trips, Distance, Trip value, Money in (percent) and Completion (points), absent with the true reason on a rolling window or under a channel filter (compare/period answers 400 there); sparklines on four tiles, complete days only, and Money in says why it has none; the tiers list shows 12 and counts the rest in a sentence rather than every tier; the outcome bars use the /api/kpis buckets and keep every raw status word.
- STEP 4, L. Shell restructure, gated on the skin attribute: masthead with the window, section row with Set up and freshness, view row, sticky .ctl with the .applies sentence, the livebar layout, the authbar grid (surface, swatch, as-of, settings link), and the title block. The default shell is untouched until the flip.
  - *As built, 2026-09-23 (branch reskin-foundation; details in FIX-STATUS.md "Arkiv reskin, STEP 4").* What STEP 5 and the page phase need to know. (1) **Which shell is a token**, `--pg-shell` (app.css 0, arkiv.css 1), read by `shell.js shellContract()`; `buildShell()` MOVES the existing nodes (nothing is re-created, so every listener and every id survives, #fRangeLabel included) into the masthead (`#mast`, window in `#mastWin`/`#mastWinSub`), the section row (`#secRow`: #nav, then #settingsLink relabelled "Set up", then #freshness), #sectabs, #authBanner, the control bar (#filters gains `.ctl`, #themeBtn and the sentence `#fApplies`), #todayNow and the title block (`.topbar`), in that order before #view and #pageFoot, and stamps `#app.ak-shell`. arkiv.css keeps `#app` invisible until that class is on it. The old skin's DOM is index.html's, byte for byte (measured below). (2) **The first render waits for the stylesheets** (`shell.js whenStyled`, called at boot): a script-inserted module can run before a parser-inserted `<link>` has loaded, and with arkiv.css held back 1.5s the first render read BOTH tokens as the old skin's 0 — the old shell and STEP 3's classic #overview under the new stylesheet. A sheet slower than the 4s cap renders without it and again when it lands. **Then index.html was changed to have the PARSER write the module in** (deferred), which fixes a boot race STEP 1 introduced — its inline script after the stylesheets held `<body>` back while an appended module ran without it, so with app.css slow both builds threw on their first DOM access and showed a blank page, in both skins (FIX-STATUS SH16, test/boot_order) — and which also orders the first run after the parser's stylesheets, leaving the wait as the second line. Never append the module again. (3) **`render()` scrolls to the top of the sheet** under the new shell, not to #view, which would hide the title block. (4) Markup that changes under the shell, each branch token-gated with the old branch byte-identical: authBanner (four cells, the channel swatch via `swatch(provider)`, `rows[].surface` under the key, "as of" the LATEST `checked_at` of the rows shown — none printed when no row has one — and a "Set up → credentials" link), todayNow (value + `.lb-l` label cells, `.lb-hl` on trip value, the scope caption, and the notes that lived only in `host.title` in a closed `<details>`; `host.title` is still built), freshness (one line joined by `.fr-sep`). (5) **The `.applies` sentence** is `appliesSentence(view)`: it names exactly the controls data.js hides and gives each page's reason from data.js's own comments (`APPLIES_WHY`); compliance, insights and retention take the shared "none of them changes what it shows". (6) **The masthead window** prints two Dubai days only for a rolling window (the client computed them), the name alone for a calendar period (the server's calendar resolves it), and "No window applies here" on a hidesRange() page. (7) **Deviations**, each for a reason: the theme button keeps its word ("◐ system") rather than an icon alone, because the word is the state; #tzNote stays in the bar as it was (the masthead now always says Dubai time); the Fraunces preload and the theme-color metas are STEP 5's, as STEP 1 recorded. (8) render-audit's colour checks now read the shell's chrome too (every child of `#app.ak-shell > .main` but #view and #pageFoot); clean on production and the mock. (9) **The fold is the banner's**: with production's three stopped rows (252px) #view starts at 635px on #overview at 1440×900 and the first tile at 897; without it, ~383. Clipping the banner's detail is an operator ruling, not a restyle (FIX-STATUS NOT DONE).
...
- DARK MODE. The live app has one: system/light/dark via applyTheme, a pre-paint stamp on the phone, two token blocks. The design is validated against #FFFFFF only and defines no dark set. CABMAN #6B259F and the ink-on-wash pairings cannot be reused on a dark ground. Options: (a) solve an Arkiv-dark set with the same validator in dark mode before the flip (M); or (b) ship Arkiv light only, with the toggle saying dark is not available yet. (b) removes a working feature, so it is the operator's call. Never keep the old dark palette under the new skin: that would be two colour laws.
  - *As built, 2026-09-23 (ruling 3 chose (a); branch reskin-foundation; details in FIX-STATUS.md "Arkiv reskin, dark mode" and the evidence in docs/ARKIV-DARK.md).* The dark set is in tokens.js and is generated under the old skin's own theme mechanism, scoped to the skin. It passes every validator gate light passes except one: channel × semantic, where Hotel × negative is CVD 7.0 (WARN, legal with the glyph-and-sign and direct-label rules the law already has). Uber is the muted blue-violet in dark (C 0.108), and the positive moved to 161.7° to keep the semantic pair at 8.1. Later steps need to know three things. (1) A rule that picks paper or ink for a coloured fill must use a per-theme token, like `--on-c-*`, because paper and ink swap lightness. (2) charts.js's SPEC §5 hatch should use `--hatch-a` (0.45 light, 0.53 dark). (3) index.html now stamps the stored theme before the first paint on the desktop build too.
